...
import {
  ...
  DeleteOnDB,
  GetLastLogOnDB,
} from "../../services/routesData/routesData.js";
...
export default ({ route }) => {
  ...
  async function GetLastSyncRoutes() {
    ...
    for (let i = 0; i < ListToSync.length; i++) {
      const ret = await GetLastLogOnDB(
        GText.infoDB.Table.Log.fields.route,
        Routes[i].name,
        GText.infoDB.Table.Log.fields.action,
        GText.Log.actions.sync
      );
      const ret1 = await GetLastLogOnDB(
        GText.infoDB.Table.Log.fields.route,
        Routes[i].name,
        GText.infoDB.Table.Log.fields.action,
        GText.Log.actions.delete
      );
  ...
  function handleButtonModal() {
    let SelectedData = [];
    data.map((obj) => {
      obj.checked && SelectedData.push(obj.name);
    });
    ModalRef.current.toggle();
    Origin === GText.Config
      ? handleDeleteDataOnDB(SelectedData)
      : navigate.navigate(GText.Syncing, {
          routes: SelectedData,
          origin: GText.SelectToSync,
        });
  }
  function toggleChecedkAll(closelist) {
    ...
    Origin === GText.Config
      ? closelist &&
        navigate.reset({ routes: [{ name: Origin, params: undefined }] })
  ...
  function OpenConfirmation(data) {
    if (ItensChecked.current > 0) {
      ModalRef.current.setLabel(
        Origin === GText.Config
          ? GText.labelModalDeleteOnDB
          : GText.labelModalSyncItens
      );
      ModalRef.current.toggle();
  ...
  return (
    <Container>
      <Header
        title={
          Origin === GText.Config ? GText.SelectToDelete : GText.SelectToSync
        }
        ...
        {data.map((item, key) => {
          return (
            <ViewLine
              key={key}
              onPress={() => {
                handleOnChangeCheckBox(!item.checked, item.name);
              }}
              style={{
                backgroundColor:
                  item.checked === true ? Global.bluelight2 : Global.white,
              }}
            >
              <CheckBox
                value={item.checked === true ? true : false}
                onValueChange={(newValue) =>
                  handleOnChangeCheckBox(newValue, item.name)
                }
              />
              <View>
                <TextStyled>{item.name}</TextStyled>
                {item.Deleted !== "" ? (
                  <TextStyled style={{ fontSize: 12 }}>
                    {" "}
                    {GText.messageDeleted + `:  ${item.Deleted}`}{" "}
                  </TextStyled>
                ) : (
                  <TextStyled style={{ fontSize: 12 }}>
                    {" "}
                    {GText.messageLastSync + `:  ${item.LastSync}`}{" "}
                  </TextStyled>
                )}
              </View>
            </ViewLine>
      ...
      <Line style={{ display: ItensChecked.current > 0 ? "flex" : "none" }}>
        <Button
          name={Origin === GText.Config ? Global.IconTrash : Global.IconSync}
          size={40}
